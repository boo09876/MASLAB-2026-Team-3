...
import utils
import kinematics
from controls import KalmanFilter, PIDFController, FilteredPIDFController
...
class VectorCalculator:
    def __init__(self):
        ...
        self.translationalPIDF = PIDFController(0, 0, 0, 0) 
        self.translationalIntegral = PIDFController(0, 0, 0, 0)
        ...
        self.k_curvature = 0    # curvature feedforward scaling
        self.k_heading = 0.03
    ...
    def breakFollowing(self):
        ...
        self.translationalPIDF.reset()
        self.translationalIntegral.reset()
        
        self.driveVector = Vector()
        self.headingVector = Vector()
        self.translationalVector = Vector()
        self.centripetalVector = Vector()
        self.correctiveVector = Vector()
    ...
    def getTranslationalCorrection(self):
        translationalVector = self.translationalError.copy()

        if not (self.currentPath.isAtParametricEnd() or self.currentPath.isAtParametricStart()):
            translationalVector = translationalVector.minus(Vector(translationalVector.dot(self.currentPath.closestPointTangentVector.normalize()), self.currentPath.closestPointTangentVector.theta))
            
            self.secondaryTranslationalIntegralVector = self.secondaryTranslationalIntegralVector.minus(Vector(self.secondaryTranslationalIntegralVector.dot(self.currentPath.closestPointTangentVector.normalize()), self.currentPath.closestPointTangentVector.theta))
            self.translationalIntegralVector = self.translationalIntegralVector.minus(Vector(self.translationalIntegralVector.dot(self.currentPath.closestPointTangentVector.normalize()), self.currentPath.closestPointTangentVector.theta))

        self.translationalIntegral.updateError(translationalVector.magnitude)
        self.translationalIntegralVector = self.translationalIntegralVector.plus(Vector(magnitude=self.translationalIntegral.run() - self.previousTranslationalIntegral, theta=translationalVector.theta))
        self.previousTranslationalIntegral = self.translationalIntegral.run()

        self.translationalPIDF.updateFeedForwardInput(1)
        self.translationalPIDF.updateError(translationalVector.magnitude)
        translationalVector.magnitude = self.translationalPIDF.run()
        translationalVector = translationalVector.plus(self.translationalIntegralVector)

        self.translationalVector.magnitude = utils.clamp(self.translationalVector.magnitude, 0, self.maxPowerScaling)
        self.translationalVector = translationalVector.copy()

        return translationalVector
    
    def getCentripetalForceCorrection(self):
        if self.averageVelocity.xComponent == 0:
            return Vector()
        yPrime = self.averageVelocity.yComponent / self.averageVelocity.xComponent
        yDoublePrime = self.averageAcceleration.yComponent / self.averageAcceleration.xComponent
        curvature = (yDoublePrime) / (math.sqrt(1 + yPrime**2))**3

        self.centripetalVector = Vector(utils.clamp(self.centripetalScaling * self.mass * self.velocity.dot(self.currentPath.closestPointTangentVector.normalize())**2 * curvature, -self.maxPowerScaling, self.maxPowerScaling), self.currentPath.closestPointTangentVector.theta + (math.pi/2) * utils.signum(self.currentPath.closestPointNormalVector.theta))
        return self.centripetalVector
    
    def getCorrectiveVector(self):
        centripetal = self.getCentripetalForceCorrection()
        translational = self.getTranslationalCorrection()
        corrective = centripetal.plus(translational)

        if corrective.magnitude > self.maxPowerScaling:
            return centripetal.plus(translational.times(utils.findNormalizingScaling(centripetal, translational, self.maxPowerScaling)))
        
        self.correctiveVector = corrective.copy()

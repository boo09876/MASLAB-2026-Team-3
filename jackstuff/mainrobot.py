...
from robot_timer import Timer
from motion_profile import MotionProfile
from path import Path, PathPoint
from calculators import ErrorCalculator, VectorCalculator
from bezier_stuff import BezierCurve
...
class Robot:
    ...
    def run_drive(self, drivePowers):
        print(f"Left Power: {drivePowers[0]}, Right Power: {drivePowers[1]}")
    ...
    def servo_off(self):
        self.raven_board.set_servo_off(servo)
    ...
    def outtake(self, timeDifferenceBetween=0.075):
        raven_board.set_motor_mode(self.left_intake, Raven.MotorMode.POSITION)
        raven_board.set_motor_mode(self.right_intake, Raven.MotorMode.POSITION)
        raven_board.set_motor_pid(self.left_intake, 4, 0, 0.1, 100)
        raven_board.set_motor_pid(self.right_intake, 4, 0, 0.1, 100)
        
        startLTicks = raven_board.get_motor_encoder(self.left_intake)
        startRTicks = raven_board.get_motor_encoder(self.right_intake)
        ...
        while abs(lticks - startLTicks) < 3200:
            ...
            raven_board.set_motor_target(self.left_intake, lticks - 200)
            raven_board.set_motor_target(self.right_intake, rticks + 200)
